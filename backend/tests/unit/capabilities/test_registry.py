...
def test_unknown_version_returns_capability_not_found() -> None:
    ...
    with pytest.raises(CapabilityNotFoundError):
        CapabilityService(registry).get("chunk_strategy", "token", "999")
...
def test_registered_schema_is_deeply_immutable() -> None:
    ...
    stored = registry.get("chunk_strategy", "token", "1")
...
def test_builtin_catalog_registers_supported_model_providers_and_types() -> None:
    ...
    compatible = registry.get("model_provider", "openai_compatible", "1")
...
def test_builtin_catalog_registers_parser_routes_and_input_types() -> None:
    ...
    html = registry.get("input_type", "html", "1")
    ...
    text = registry.get("input_type", "txt", "1")
    ...
    archive = registry.get("input_type", "zip", "1")
    ...
    mineru = registry.get("parser", "mineru_precision_api", "1")
